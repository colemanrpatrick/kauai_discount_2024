...
//___________________| Reservation UI V2.6|______________________
...
let cutOffMessage = (page) => {
  ...
  $windowDiv.innerHTML =
    "<div><span class='material-symbols-outlined'>schedule</span><br><p>This activity is not available after " +
    cartData.Availabilities[0].Cutoff +
    ". Please select another date.</p></div>";
...
let setDisabledAndHighlited = (
  ...
  if (highlightedDates !== undefined) {
    ...
    Array.prototype.forEach.call(highlightedDates, function (item, index) {
    ...
    let organizeDates = (date) => {
      ...
      let disabledCalendarDates =
        formattedCalendarDates(disabledDates).indexOf(mdyDate) !== -1;
...
let setDatepickerKey = ($id, $description) => {
  //$id,$description
  ...
  if (highlightedDates) {
    for (let i = 0; i < highlightedDates.length; i++) {
      if (highlightedDates[i].month == alphaMonth[$month]) {
        setDatepickerKey(
          highlightedDates[i].dateId,
          highlightedDates[i].dateTitle
        );
      } else {
        // do nothing
      }
...
let showPackageGroupings = (page) => {
  ...
  Array.prototype.forEach.call($sortedPrices, function (item, index) {
    if (
      ...
      item.Grouping !== "Add Infant"
    ) {
      if (item.salePrice !== 0) {
        $groups.push([item.Grouping, item.ListPrice, item.Saleprice]);
      }
    }
  });
  ...
  Array.prototype.forEach.call($groups, function (item, index) {
    ...
    if (item[2] == 0) {
      ...
      <span class='price'>From <span>$${item[1]}</span>
      </div>
      ...
      packageButton.innerHTML = `<div class="group-left">
      <span class='title'>${item[0]}</span>
      <span class='price'>From <span class='strike'>$${item[1]}</span> <span class="from">$${item[2]}</span></span>
      </div>
      <div class='radio unchecked'>  
      <span class="material-symbols-outlined">check</span>    
      </div>`;
    ...
    packageButton.addEventListener("click", () => {
      for (
        ...
        document.getElementsByClassName("radio")[i].className =
          "radio unchecked";
      }
...
let displayPage1 = ($group) => {
  ...
  if (
    ...
    document.getElementById("page0") !== null
  ) {
    document
      .getElementById("reservation-controls")
...
let displayPage2 = ($back) => {
  ...
  if ($group) {
    ...
    //forEach
    let groupPriceContainers =
      document.getElementsByClassName("price-container");
    let groupPriceCategoriesArray = [];

    Array.prototype.forEach.call(groupPriceContainers, function (item, index) {
      let groupPriceCategories = item.getAttribute("data-category");
      groupPriceCategoriesArray.push(groupPriceCategories);
    });

    groupPriceCategoriesArray = groupPriceCategoriesArray.filter(
      (item, index, array) => {
        return array.findIndex((i) => i[0] === item[0]) === index;
      }
    );
...
let displayPage3 = () => {
  ...
  document
    .getElementById("reservation-controls")
    .appendChild(createButton("prev-3", "back", "prev-btn"));

  document.getElementById("prev-3").addEventListener(
    "click",
    function () {
      clearReservationPage("page2");
      displayPage2($group);
      //clearReservationPage("page3");
    },
    false
  );
